...
public class LxitJiraManager {
  ...
  private static void createGroups() {
    ...
    for (Iterator<String> iterator = classList.iterator(); iterator.hasNext();) {
      ...
      try {
        JiraTools.getJiraSoapService().deleteGroup(JiraTools.getAuth(), getGroupNameFromClassName(className), null);
      } catch (Exception e) {
        e.printStackTrace();
      }
    ...
    for (Iterator<String> iterator = classList.iterator(); iterator.hasNext();) {
      String className = iterator.next();
      try {
        JiraTools.getJiraSoapService().createGroup(JiraTools.getAuth(), getGroupNameFromClassName(className), null);
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
